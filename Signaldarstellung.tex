...
%\documentclass[4pt,a4paper]{scrartcl} %USE IN CASE OF EMERGENCY  geschafft! emergency not needed
...
\titleformat{\section}{\Large \bfseries}{\thesection .}{0.5em}{\color{purple}}[\color{purple} \hrule \hrule]
...
%=======================================================================
	...
	\end{itemize}
%	$x(t) \ra x(t - 1)$ Verschiebung um $1$ in $x$-Richtung \\
%	$x(t) \ra x(2 t)$ Stauchung um $2$ in $x$-Richtung \\
%	$x(t) \ra x(-t)$ Spiegelung an der $y$-Achse
%	Linearität (L): $a x_1(t) + b x_2(t) \ra a y_1(t) + b y_2(t)$ \\
%	Zeitinvarianz (TI): $x(t - t_0) \ra y (t-t_0)$
...
%	\emph{Abtastung}\\
%	$\ra$ S. 10
%	$x(t) \stackrel{\text{jede} T_S}{\ra} x[t] \stackrel{\text{mit} n = \frac{t}{T_S}}{\ra} x[n]$\\
%	$\Ra x[n] = \sum\limits_{n = -\Delta}^\Delta x(t) \cdot \delta[t - n T_s]$\\
%	$T_S$: Abtastweite
...
%	Fouriertransformation entspricht dem Grenzfall $\omega_0 \ra 0$ der Fourierreihe, es sind also beliebige Frequenzen möglich.\\
%	
...
%	
%	Achsensymmetrie: $X(-\omega) = X^*(\omega) \Ra |X(-\omega)| = |X(\omega)|$\\
%	
